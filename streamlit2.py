...
def main():
    ...
    query = st.text_area(f"질문을 입력하세요: ({i+1})", height=150, key = i )
    
    while True:
        # 각 질문마다 고유한 key 값을 부여
        
        if query:
            if i >= 1 and query.strip() == "":
                st.write("질문을 입력해주세요.")
                continue  # 질문이 비어 있으면 다음 루프를 진행


            
            # 사용자가 "종료" 또는 "끝"이라고 입력하면 대화 종료
            ...
            # 질문 번호를 증가시켜서 고유한 key가 계속 생성되도록 함
            i += 1  # 이 부분에서 i를 증가시켜서 각 질문마다 고유한 key 생성
